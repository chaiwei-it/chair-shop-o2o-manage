<template>
  <div class="specValue">
      ...
      <el-button @click="cancal">返回</el-button>
...
export default {
  ...
  mounted() {
    if (this.pager.specId == null || this.pager.specId === '') {
      window.history.go(-1)
    } else {
      this.getPager()
    }
  ...
  methods: {
    ...
    getPager() {
      ...
      ajax(api.specValuePager, 'POST', this.pager, callback)
    },
    ...
    handleDelete(id) {
      this.$confirm('确认删除该规格值吗?', '提示', {
        ...
        ajax(api.specValueDelete, 'POST', { id: id }, callback)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消删除'
    ...
    add(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          ajax(api.specValueAdd, 'POST', this.form, callback)
        } else {
    ...
    update(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          ajax(api.specValueUpdate, 'POST', this.form, callback)
        } else {
          console.log('error submit!!')
          return false
        }
    ...
    getDetails(id) {
      const callback = (data) => {
        ...
        this.form.specId = data.specId
        this.form.sortNum = data.sortNum
        this.form.image = data.image
      }
      ajax(api.specValueSelect, 'POST', { id: id }, callback)
    },

    vacant() {
      this.form.id = ''
      this.form.name = ''
      this.form.specId = this.$route.params.id
