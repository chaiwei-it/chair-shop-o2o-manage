<template>
  <div class="goodsClass">
    ...
    <el-table :data="tableData" border style="width: 100%">
      ...
      <el-table-column prop="keywords" label="关键字" width="120" sortable> </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getPager() {
        ...
        ajax(api.goodsClassPager, 'POST', this.pager, callback)
      },
      ...
      handleDelete(id) {
        this.$confirm('确认删除该规格吗?', '提示', {
          ...
          ajax(api.goodsClassDelete, 'POST', { id: id }, callback)
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消删除'
      ...
      add(form) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            ajax(api.goodsClassAdd, 'POST', this.form, callback)
          } else {
      ...
      update(form) {
        this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            ajax(api.goodsClassUpdate, 'POST', this.form, callback)
          } else {
            console.log('error submit!!')
            return false
          }
      ...
      getDetails(id) {
        ...
        ajax(api.goodsClassSelect, 'POST', { id: id }, callback)
      },
